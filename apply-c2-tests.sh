#!/usr/bin/env bash
#
# apply-c2-tests.sh — Batch C2, Schritt 1: nur Tests, keine Korrekturen
#
# Beweist die Befunde rund um /analytics und /history, bevor etwas
# geändert wird. Nach diesem Skript sollen Tests ROT sein.
#
# Ausführen im Wurzelverzeichnis des Repos:
#     bash apply-c2-tests.sh
#
# Idempotent. Rührt keine Anwendungslogik an — es legt genau eine neue
# Testdatei an und ändert sonst nichts.
#
set -euo pipefail
...
echo
echo "── Batch C2, Schritt 1: Beweise ────────────────────────────────"
echo

[ -d "$BE/test/integration" ] || die "'$BE/test' fehlt. Bitte zuerst Batch B anwenden."
...
ok "test/integration/analytics.test.js (8 Tests)"
...
echo "── Erwartung ───────────────────────────────────────────────────"
echo
echo "  1 GRÜN   die Kontrolle"
echo "  5 ROT    die Befunde, die C2 behebt"
echo "  2 OFFEN  isBio (klärt eine Unklarheit) und der negative days-Wert"
echo "  42 GRÜN  alles aus Batch A, B und C1"
echo
echo "  Ist die KONTROLLE rot, stimmt eine Annahme über /analytics nicht —"
echo "  dann bitte die Ausgabe schicken, bevor wir etwas reparieren."
echo "  Schlägt die Zählerprobe im Speicher-Test fehl, greift dort das"
echo "  Messverfahren nicht; auch das bitte schicken."
echo
