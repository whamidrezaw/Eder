...
// ── Abschlussprüfung, noch in der Planphase ───────────────────────
{
  ...
  if (sh) {
    const nurCode = sh.aktuell.split('\n').filter(l => !/^\s*\/\//.test(l)).join('\n');
    const anzahlJson = (nurCode.match(/await res\.json\(\)\.catch/g) || []).length;
    if (anzahlJson !== 1) {
      fehler.push({
        name: 'Abschlussprüfung',
        datei: 'frontend/assets/shared.js',
        hinweis: `api() wertet die Antwort ${anzahlJson}-mal aus, erwartet genau 1 — ` +
                 'ein zweites res.json() auf derselben Antwort liefert einen Fehler',
        ausschnitt: umgebung(sh.aktuell, /res\.json\(\)\.catch/, 12)
      });
    }
    if (/^\s*localStorage\.clear\(\);\s*$/m.test(nurCode) &&
        !/anzeigeEinstellungen/.test(nurCode)) {
      fehler.push({
        name: 'Abschlussprüfung',
        datei: 'frontend/assets/shared.js',
        hinweis: 'localStorage.clear() steht noch ungeschützt in der Datei',
        ausschnitt: umgebung(sh.aktuell, /localStorage\.clear/)
      });
    }
  }
}
