#!/usr/bin/env bash
#
# apply-c1-tests.sh — Batch C1, Schritt 1: nur Tests, keine Korrekturen
#
# Beweist die Fehler rund um den Tagesabschluss, bevor irgendetwas
# repariert wird. Nach diesem Skript sollen Tests ROT sein — das ist der
# Sollzustand. Die Korrekturen kommen im nächsten Skript.
#
# Ausführen im Wurzelverzeichnis des Repos:
#     bash apply-c1-tests.sh
#
...
echo
echo "── Batch C1, Schritt 1: Beweise ────────────────────────────────"
echo

[ -d "$BE/test/integration" ] || die "'$BE/test' fehlt. Bitte zuerst apply-batch-b.sh ausführen."
...
ok "test/integration/daily-close.test.js (10 Tests)"
...
echo "── Erwartung ───────────────────────────────────────────────────"
echo
echo "  3 Tests GRÜN  (2 Kontrollen + Leitplanke für manuelle Berichte)"
echo "  7 Tests ROT   (die Befunde, die C1 behebt)"
echo "  28 Tests aus Batch A/B bleiben GRÜN"
echo
echo "  Wenn eine der beiden KONTROLLEN rot ist, stimmt eine meiner"
echo "  Annahmen über closeDay() nicht — dann bitte die Ausgabe schicken,"
echo "  bevor wir etwas reparieren."
echo
